...
export default function AddEvent(props) {
  ...
  function Time() {
    ...
    if (useTime) {
      ...
      return (
        <React.Fragment>
          ...
          <input
            className="time"
            type="text"
            placeholder={moment().hour() + ":" + moment().minute()}
            onBlur={e => {
              console.log(event);
              ...
              if (
                ...
                event.start.date.setHours(
                  moment(e.currentTarget.value, "HH-mm").format("HH")
                );
                event.start.date.setHours(
                  moment(e.currentTarget.value, "HH-mm").format("mm")
                );

                event.end.date.setHours(
                  moment(e.currentTarget.value, "HH-mm").format("HH")
                );
                event.end.date.setHours(
                  moment(e.currentTarget.value, "HH-mm").format("mm")
      ...
      return <button onClick={() => setUseTime(true)}>Specify time</button>;
    }
  }
  return (
    <div className="settings addEvent" key={+new Date()}>
      ...
      <div className="settingsBody">
        ...
        <select>
          {props.settings.calendar.calendarIDs.map(i => (
            <option
              onClick={e => (calendarID = e.currentTarget.value)}
              value={i.id}
            >
              {i.name}
            </option>
      ...
      <div className="saveSettings" onClick={Add}>
        Add Event
      </div>
    </div>
  );
